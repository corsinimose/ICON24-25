...
class Dataset:
    ...
    def save_to_files(self):
        os.makedirs(os.path.dirname(self.training_path), exist_ok=True)

        if self.training is not None:
            self.training.to_csv(self.training_path, index=False)
            print(f"Dataset di training salvato in {self.training_path}")
        else:
            print("\nATTENZIONE: Nessun dataset di training da salvare.")

        if self.test is not None:
            self.test.to_csv(self.test_path, index=False)
            print(f"Dataset di test salvato in {self.test_path}")
        else:
            print("\nATTENZIONE: Nessun dataset di test da salvare.")


    def load_from_files(self):
        if os.path.exists(self.training_path):
            self.training = pandas.read_csv(self.training_path)
            print(f"Dataset di training caricato da {self.training_path}")
        else:
            print(f"\nATTENZIONE: Nessun dataset di training in {self.training_path}")

        if os.path.exists(self.test_path):
            self.test = pandas.read_csv(self.test_path)
            print(f"Dataset di test caricato da {self.test_path}")
        else:
            print(f"\nATTENZIONE: Nessun dataset di test in {self.test_path}")