...
def runbn(dataset: Dataset, mode: str = "normal", method: str = "ml"):
    ...
    if ask_question("Hai effettuato un esame gastrointestinale?"):
    ...
    probability = probability = (result["p"])[1] * 100
    print("La probabilità di avere la malattia è %.2f" % probability)
    if probability >= 50: print("È consigliabile recarsi da un medico per ulteriori accertamenti")
...
class DiagnosticsBN:
    ...
    def learn_from_dataset(self, dataset, method):
        if dataset is None or dataset.empty:
            raise ValueError("\nDataset di training non presente. Creare e caricare prima il dataset.")
